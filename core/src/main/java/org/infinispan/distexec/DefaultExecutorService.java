...
public class DefaultExecutorService extends AbstractExecutorService implements DistributedExecutorService {
    ...
    protected <T> RunnableFuture<T> newTaskFor(Callable<T> callable) {
        ...
        DistributedExecuteCommand<T> executeCommand = factory
                .buildDistributedExecuteCommand(callable, getAddress(), null);
    ...
    public <T> Future<T> submit(Address target, DistributedTask<T> task) {
        ...
        if (!members.contains(target)) {
            throw new IllegalArgumentException("Target node " + target + " is not a cluster member, members are "
                    + members);
        }
    ...
    public <T, K> Future<T> submit(DistributedTask<T> task, K... input) {
        ...
        if (inputKeysSpecified(input)) {
            ...
            Address me = getAddress();
            DistributedExecuteCommand<T> c = factory.buildDistributedExecuteCommand(task.getCallable(), me,
                    Arrays.asList(input));
            ArrayList<Address> nodes = new ArrayList<Address>(nodesKeysMap.keySet());
            DistributedTaskPart<T> part = createDistributedTaskPart(task, c, Arrays.asList(input),
                    selectExecutionNode(nodes), 0);
            part.execute();
            return part;
        } else {
    ...
    protected <T, K> DistributedTaskPart<T> createDistributedTaskPart(DistributedTask<T> task,
                                                                      DistributedExecuteCommand<T> c, List<K> inputKeys, Address target, int failoverCount) {
    ...
    private <T, K> void checkExecutionPolicy(DistributedTask<T> task, Map<Address, List<K>> nodesKeysMap, K... input) {
        if (nodesKeysMap == null || nodesKeysMap.isEmpty()) {
            throw new IllegalStateException("DistributedTaskExecutionPolicy " + task.getTaskExecutionPolicy()
                    + " for task " + task + " returned invalid keysToExecutionNodes " + nodesKeysMap
    ...
    protected <K> Map<Address, List<K>> keysToExecutionNodes(DistributedTaskExecutionPolicy policy, K... input) {
        ...
        for (K key : input) {
            ...
            if (usingREPLMode) {
                ...
                Collections.shuffle(members); // TODO: this should be smarter.
                // Perhaps keep an index of last
                // member, and cycle which
                // member we picked. This is a
                // O(N) operation that mutates
                // the list of members!
    ...
    private class DistributedTaskPart<V> implements NotifyingNotifiableFuture<V>, RunnableFuture<V> {
         ...
         * @param failoverCount
         */
        public DistributedTaskPart(DistributedTask<V> task, DistributedExecuteCommand<V> command, List<Object> inputKeys,
                                   Address executionTarget, int failoverCount) {
        ...
        private V failoverExecution(final Exception cause, long timeout, TimeUnit unit) throws Exception {
            ...
            DistributedTaskPart<V> part = createDistributedTaskPart(owningTask, distCommand, getInputKeys(), target,
                    failedOverCount);
            part.execute();
        ...
        V retrieveResult(Object response) throws Exception {
            ...
            // these two should never happen, mark them with
            ...
            if (mapResult.size() == 1) {
                ...
                // should never happen as we send DistributedTaskPart to one
                // node for
