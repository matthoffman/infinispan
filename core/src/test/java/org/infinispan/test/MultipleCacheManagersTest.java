...
/**
 * Base class for tests that operates on clusters of caches. The way tests extending this class
 * operates is:
 *
 * <ol>
 * <li>created cache managers before tests start. The cache managers are only created once
 * <li>after each test method runs, the cache instances are cleared
 * <li>next test method will run on the same cacheManager instance. This way the test is much
 * faster, as CacheManagers are expensive to create.
 * </ol>
 *
 * If, however, you would like your cache managers destroyed after every <i>test method</i> instead
 * of the </i>test class</i>, you could set the <tt>cleanup</tt> field to
 * {@link MultipleCacheManagersTest.CleanupPhase#AFTER_METHOD} in your test's constructor. E.g.:
 *
 * <pre>
 * <p/>
